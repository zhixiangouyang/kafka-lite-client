...
public class KafkaProducerOrderTest {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            // 创建多个发送线程，提高生产速度
            int producerThreads = 1; // 使用2个线程并行生产消息
                    ...
                    try {
                        while (System.currentTimeMillis() - startTime < testDurationMs) {
                            ...
                            if (producer.getQueueSize() < config.getMaxQueueSize() * 0.8) {
                                ...
                                // 计算消息内容长度，确保总长度不超过限制
                                int maxContentLength = messageSizeBytes - orderPrefix.length();
                                String messageContent = messageTemplate.substring(0, Math.min(messageTemplate.length(), maxContentLength));

                                // 组合最终消息：有序前缀 + 消息内容
                                String messageValue = orderPrefix + messageContent;

                                ProducerRecord record = new ProducerRecord(
                                        "order-test-topic-4", // 使用您指定的topic
                                        "key" + localIndex,
                                        messageValue
                                );
